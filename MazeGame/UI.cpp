...
void UI::drawStartUI(bool playerSelected, bool difficultySelected, bool playerMode, Difficulty difficulty) {
    setbkcolor(RGB(255, 165, 0));  // 设置背景颜色为黄色
    cleardevice();
    ...
    solidrectangle((width) / 2 - 80, 50, (width) / 2 + 80, 200);
    outtextxy((width) / 2 - 40, 60, _T("选择模式"));
    ...
    // 绘制难度选择框
    solidrectangle((width) / 2 - 80, 250, (width) / 2 + 80, 400);
    outtextxy((width) / 2 - 40, 260, _T("选择难度"));
